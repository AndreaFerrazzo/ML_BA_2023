...
After filtering extreme values we can observe that most of the recipes have between 200 and 600 calories. By checking with the histogram and density plot the distribution of calories, we observe that indeed most of the recipes have less than 1000 calories.
...
ingredients_df_bin %>% 
  ...
  plot_correlation(title = 'Correlation of rating with the "binary" ingredients categories')
```
...
We also noticed that 5 ingredients variables are actually present in none of the recipes we have. We therefore remove those 5 ingredient variables, leaving us with 288 binary ingredient variables.
```{r}
...
all_ingredients <- all_ingredients[!all_ingredients %in% ingredients_to_remove]
```
...
ggplot(aes(x=sum_season)) +
  geom_bar() + 
  labs(x = "Number of Seasons", y = "Count", title = "Number of different seasons per recipe")
...
seasons_df %>% 
  ...
  plot_correlation(title = "Corrplot of seasons with rating")
```
...
type_df %>% 
  ...
  geom_bar()+
  labs(x = "Number of recipe types", y = "Count", title = "Number of different recipe types variables per recipe")
...
type_df %>% 
  ...
  plot_correlation(title = "Corrplot of recipe types relative to rating")
```
...
countries_df %>% 
  ggplot(aes(x=sum_type)) +
  geom_bar()+
    labs(x = "Number of Countries per recipe", y = "Count", title = "Number of different countries variables per recipe")
```


# Unsupervised Learning EDA

## Exploratory PCA Analysis - ALL 680 variables
...
## Exploratory PCA Analysis - Selected
...
```
The results do not substantially differ from the analysis with 680 variables.

## Exploratory PCA Analysis - Nut + Total + Bin
...
```
Here we can observe that calories and fat are positively correlated with the first dimension, whereas fruits_bin and total_fruits are negatively correlated with the second dimension.
...
We notice that nutritional values, meats_bin, vegetables_bin, total_dessert, total_meat and a few other engineered variables make a significant contribution to the first dimension. 
...
```
Here we see that most points are distributed on the positive side of dimension 1 and equally spread on dimension 2. We notice extreme values on the top right, where dimension 1 and dimension 2 are highly positive.
...
```
Here the first 10 dimensions account for 70.9% of total explained variance.
...
```
This is an overview of the 10 selected dimensions. However, it is hard to detect clear patterns.
...
```{r Clustering 4, fig.height=10, fig.width=10}
...
### Interpretation of the clusters
Here we analyze the clusters by looking at the distribution of the features within each cluster
```{r Clustering 9, fig.height=10, fig.width=10}
...
### Interpretation of the clusters
Here we analyze the clusters by looking at the distribution of the features within each cluster
```{r Clustering 12, fig.height=7, fig.width=10}
