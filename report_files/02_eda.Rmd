...
#this is way faster than the code below
recipes <- recipes %>% 
  drop_na()
...
In order to decide whether we want to analyse with or without general categories, let's see how many observations would be left with if we remove all the obs for which we have a general category at 1, and all specific ingredients in that category is set to 0.
...
recipes_plot %>% 
  ggplot(aes(x=protein, y=fat, color=rating, alpha=rating, size=rating)) +
  geom_point() + 
  scale_color_gradientn(colours = rainbow(5))


# We tried various representations of protein and fat according to rating without being able to clearly show a pattern.
