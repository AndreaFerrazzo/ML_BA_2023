...
The numner of ingredients per recipe is more or less normally distributed, with a mean around 4.75.
...
## Investigating each category relative to rating
...
----
## Left joins to aggregate data
```{r}

ingredients_df_full <- ingredients_df %>%
  mutate(total_ingredients = rowSums(select(., c(all_of(all_ingredients)))),
         total_meat = rowSums(select(., c(all_of(all_meats)))),
         total_vegetables = rowSums(select(., c(all_of(vegetables_vec))))
         )
...
ingredients_select <- ingredients_df_full %>% 
  select(ID, total_ingredients, total_meat, total_vegetables)
...
# Meat and proteins
recipes_full %>% 
  ggplot(aes(x=meats_bin, y=protein)) +
  geom_bar(stat = "identity") +
  ggtitle("Relationship between meat and proteins") +
  xlab("Meats") +
  ylab("Proteins") 

# We also want to show which kinds of meat specifically have a high number of proteins


# Seafood and sodium
recipes_full %>% 
  ggplot(aes(x=seafood_bin, y=sodium)) +
  geom_bar(stat = "identity") +
  ggtitle("Relationship between seafood and sodium") +
  xlab("Seafood") +
  ylab("Sodium") 

# We also want to show which kinds of seafood specifically have a high number of sodium

# Carbs and calories
recipes_full %>% 
  ggplot(aes(x=carbs_bin, y=calories)) +
  geom_bar(stat = "identity") +
  ggtitle("Relationship between food high in carbs and calories") +
  xlab("Carbs") +
  ylab("Calories") 

# We also want to show which kinds of carbs specifically have a high number of calories
