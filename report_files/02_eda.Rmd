...
# Calories histogram
histogram_calories <- recipes_plot %>% 
  filter(nutrition == "calories", n_value <= 7000) %>% 
...
grid.arrange(boxplot_protein, histogram_protein, density_protein, ncol=1, nrow =3)
...
grid.arrange(boxplot_fat, histogram_fat, density_fat, ncol=1, nrow =3)
```
...
The boxplots below give us similar information about the amount of recipe which contain at least one ingredient in each category. The only category for which an ingredient is present at least once in more than 50% of the recipes is vegetables.
...
ingredients_df_bin %>% 
  ...
  plot_correlation()
```
From this graph we notice that recipes with rating at 3.75 have slightly positive relationship with carbs and vegetables, whereas recipes with rating at 5 have slightly negative relationship with the same features. We assume then that recipes containing vegetables and carbs tend to be less appreciated.
...
```
It's hard to interpret the results clearly with so many rating, let's try to reduce it to 2 category: rating above 4 vs rating below 4.
...
```
We not have only 2 categories: recipes with rating above 4 and recipes with ratings below 4. There is no clear relationship in those graphs either, and this confirms the correlation results that we have found above.
