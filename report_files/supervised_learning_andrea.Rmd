...
```{r}
#train control for with upsampling twoclass summary
trCtrl_up_twoClass <- trainControl(method = "cv",
                       summaryFunction = twoClassSummary,
                       classProbs = TRUE,
                       number = 5,
                       sampling = "up"
...
### Logistic Regression - unbalanced - Nutritional Values Only
...
### SVM - unbalanced - Nutritional Values Only
...
The validation accuracy stands at 42.8%, which is not satisfying considering that it is computed on the training set. The next step consists in creating a grid of values for the cost that we want to try and pass to the argugment tuneGrid.
...
#### Linear SVM - balanced - Nutritional Values and Total Bin
...
#### Radial SVM - balanced - Nutritional Values and Total Bin
...
Best K if we look at both accuracy is 81
...
```{r}
knn_all_up_pred <- predict(knn_all_up, newdata = test_bin, type = "raw")

confusionMatrix(reference = test_bin$rating_bin, data = knn_all_up_pred)
```

### KNN - balanced - Nut

```{r}
train_temp <- train_bin_Nut

test_temp <- test_bin_Nut
```
...
```{r}
knn_Nu_up_pred <- predict(knn_Nu_up, newdata = test_bin_Nut, type = "raw")

confusionMatrix(reference = test_bin_Nut$rating_bin, data = knn_Nu_up_pred)
```

### KNN - balanced - NuTo

```{r}
train_temp <- train_bin_NuTo

test_temp <- test_bin_NuTo
```
...
knn_NuTo_up <- caret::train(rating_bin ~.,
                ...
                method = "knn",
                trControl = trCtrl_down,
                tuneGrid= data.frame(k = seq(1, 150,by = 5))#initially checked on 1-151 range, best was 139
                )
knn_NuTo_up
```
...
```{r}
knn_NuTo_up_pred <- predict(knn_NuTo_up, newdata = test_bin_NuTo, type = "raw")

confusionMatrix(reference = test_bin_NuTo$rating_bin, data = knn_NuTo_up_pred, positive="good")
```
...
```{r}
knn_Bi_up_pred <- predict(knn_NuBi_up, newdata = test_bin_NuBi, type = "raw")

confusionMatrix(reference = test_bin_NuBi$rating_bin, data = knn_Bi_up_pred, positive="good")
```
...
#confusion matrix 
confusionMatrix(data = rf_Nut_pred, reference = test_bin_Nut$rating)
```
...
#confusion matrix 
confusionMatrix(data = rf_NuBi_pred, reference = test_bin_NuBi$rating)
```
...
#confusion matrix 
confusionMatrix(data = rf_NuTo_pred, reference = test_bin_NuTo$rating)
```
...
#confusion matrix 
confusionMatrix(data = rf_Tot_pred, reference = test_bin_Tot$rating)
```
...
analysis_all <- recipes %>% 
  ...
  mutate(rating_bin = as.factor(ifelse(rating<4, 0, 1)), across(all_of(all_ingredients), as.factor)) %>% 
  select(-rating) %>% 
...
```{r}
#make predictions
rf_NuBi_pred <- predict(rf_down_fit, newdata = test_bin_NuTo)

#confusion matrix 
confusionMatrix(data = rf_down_pred, reference = test_bin_NuTo$rating)
```
...
#train the model
set.seed(12)
xgb_down_fit <- xgboost(data = xgb_tr,
                     params = param_list,
                     nrounds = 15,
                     verbose = FALSE)
#xgb.cv()
...
xgb_down_pred <- as.factor(ifelse(xgb_down_prob<0.561, 0, 1))
