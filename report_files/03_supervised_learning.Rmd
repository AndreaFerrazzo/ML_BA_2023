...
### Balanced data
Here we can see the results for section 5.4 on the downsampled data, with a total of 21 models that we fitted with various combinations of explanatory variables. We see that the balanced accuracy is already higher, and the sensitivity is more reasonable. Below, we investigate various aspects of these metrics and select the top-performing models. We decide to use balanced accuracy and sensitivity as the two metrics to quantitatively rank the best models.
...
summary_points <- summary_down_wide %>% 
  mutate(points_bacc = rank(balanced_accuracy), points_sens = rank(sensitivity), total_points = (points_bacc + points_sens))
...
roc_df %>%
  ...
  geom_boxplot() + 
  labs(x = "Response", y = "Probability", title = "Boxplot of probability for each response value on the test set - XGB_all")
```
...
```{r VarImp XGBoost, fig.height=12, fig.width=8}
#XGBoost variable importance
xgb_importance_matrix <- xgb.importance(model = xgb_down_fit)
xgb_importance_matrix

xgb.ggplot.importance(importance_matrix = xgb_importance_matrix %>% head(50)) +
