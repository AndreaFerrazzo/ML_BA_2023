...
xgb_down_prob <- predict(xgb_down_fit, newdata = xgb_downsamp_tr)

xgb_down_pred <- as.factor(ifelse(xgb_down_prob<0.5, 0, 1))

CM <- confusionMatrix(reference = as.factor(numeric_downsamp_tr$rating_bin), data = xgb_down_pred, positive = "1")
CM
```
...
We first look at the top-3 models with the highest balanced_accuracy
```{r}
summary_down_long %>% 
...
We now plot the top-3 models with the highest sensitivity. 
```{r}
summary_down_long %>% 
...
We now plot the top-3 models with the highest kappa
```{r}
summary_down_long %>% 
  filter(model %in% kappa_select, metric=="kappa") %>% 
  ggplot(aes(x=model, y=value, fill = model))+
  geom_bar(stat="identity", position = "dodge")+
  coord_cartesian(ylim = c(5, 15)) + 
  labs(y = "Kappa (in %)", x = "Model", title = "Top 5 models with highest Kappa")
```
```{r}
