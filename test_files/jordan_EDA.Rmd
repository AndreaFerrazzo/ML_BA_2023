...
ingredients_df <- ingredients_df %>% 
  mutate(total_ingredients = rowSums(.[,-(1:2)]), total_meat = rowSums(select(., all_of(meats_all))), total_vegetables_vec = rowSums(select(., all_of(vegetables_vec)))) %>% 
  select(ID, title, total_ingredients, total_meat, total_vegetables_vec, everything())
